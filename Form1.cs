    ...
    public partial class Form1 : Form
    {
        ...
        private void lstvBooks_ItemActivate(object sender, EventArgs e)
        {
            ...
            if (!exist)
            {
                ...
                {
                    MessageBox.Show("一次最多借兩本書！", "注意", MessageBoxButtons.OK);
                }
            }
        }
        ...
        {
            if (lstBorrow.SelectedItems.Count >= 1)
            {
                int id = lstBorrow.SelectedIndex;
                lstBorrow.Items.RemoveAt(id);
            }
            else
            {
                MessageBox.Show("請先在借書清單中選取項目！", "注意", MessageBoxButtons.OK);
            }
        }
